...
-- ============================================================
-- FK on Rentals.book_id → Books.book_id: book_id 999 does not exist
...
-- FK on Returns.rental_id → Rentals.rental_id: rental_id 999 does not exist
...
-- ============================================================
-- UNIQUE on Books.isbn: ISBN '9783869711423' already belongs to 'Romeo and Juliet'
...
-- UNIQUE on Members.email: 'crodriguez032@fiu.edu' already belongs to Chris Rodriguez
...
-- UNIQUE on Returns.rental_id: rental_id 1 already has a return record
-- (first insert succeeds if not already present; second always fails)
INSERT INTO Returns (rental_id, return_date, condition_status, notes, processed_by)
...
-- ============================================================
-- Books.title is NOT NULL: inserting NULL title
...
-- Fines.fine_amount is NOT NULL: inserting NULL fine_amount
...
-- ============================================================
-- Rentals.status ENUM is ('active','overdue','returned'): 'lost' is not valid
INSERT INTO Rentals (book_id, member_id, rental_date, due_date, status)
VALUES (1, 1, '2026-03-10', '2026-03-25', 'lost');
-- ERROR 1265 (01000) at line 52: Data truncated for column 'status' at row 1

-- Fines.paid_status ENUM is ('unpaid','partial','paid'): 'complete' is not valid
INSERT INTO Fines (rental_id, member_id, fine_amount, fine_reason, fine_date, paid_status, paid_amount)
...
-- ============================================================
-- RESTRICT: book_id 1 has child rows in Rentals, ON DELETE RESTRICT blocks this
...
-- ============================================================
-- Active member with negative max_books_allowed violates the check
INSERT INTO Members (first_name, last_name, email, phone, membership_type, registration_date, status, max_books_allowed)
...
-- ============================================================
-- due_date is before rental_date
INSERT INTO Rentals (book_id, member_id, rental_date, due_date, status)
...
-- ============================================================
-- publication_year below the lower bound (must be > 1400)
INSERT INTO Books (isbn, title, author, publisher, publication_year, category, total_copies, available_copies)
